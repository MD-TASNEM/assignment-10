  ...
  return (
    <div className="bg-white rounded-3xl overflow-hidden shadow-md hover:shadow-xl transition-all duration-300 group h-full flex flex-col">
      <div className="relative overflow-hidden">
        ...
        />
        <div className={`absolute top-4 right-4 ${statusColor} text-white text-xs px-4 py-1.5 rounded-full font-medium shadow-lg`}>
          {status}
        </div>
      ...
      <div className="p-6 flex flex-col flex-1">
        <h3 className="text-xl font-bold text-gray-900 mb-3 line-clamp-1">{title}</h3>
        <p className="text-gray-600 text-sm mb-5 line-clamp-2 flex-1">{description}</p>
        ...
        <div className="grid grid-cols-2 gap-4 text-sm mb-6">
          ...
          <div>
            ...
            <p className="font-semibold text-gray-900">{participants?.toLocaleString() || 0}</p>
          </div>
          <div>
            <p className="text-gray-500 text-xs mb-1">Impact</p>
            <p className="text-emerald-600 font-semibold">{impactMetric || 'kg CO₂'}</p>
          </div>
          <div>
            <p className="text-gray-500 text-xs mb-1">Difficulty</p>
            <span className={`inline-block px-3 py-1 text-xs rounded-full ${difficultyColor}`}>
              {difficulty || 'Medium'}
            </span>
          </div>
        </div>
